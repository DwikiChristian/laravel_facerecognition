...
@section('content')
<div x-data="prodiData()" x-init="fetchData(); fetchJurusans()" class="p-4">
    <div class="flex justify-between items-center mb-6 lg:mt-10">
        <h2 class="text-2xl font-bold text-gray-800">Daftar Program Studi</h2>
    ...
    <!-- Empty State -->
    <template x-if="!loading && prodis.length === 0">
        <div class="text-center py-8">
            <div class="text-gray-400 text-6xl mb-4">
                <i class="fas fa-graduation-cap"></i>
            </div>
            <p class="text-gray-500 text-lg">Tidak ada data program studi.</p>
            <p class="text-gray-400 text-sm">Klik tombol "Tambah Prodi" untuk menambah data baru.</p>
        </div>
    </template>
            ...
            <div class="overflow-x-auto">
                <table class="w-full table-auto">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <template x-for="(prodi, index) in prodis" :key="prodi.id">
                            <tr class="hover:bg-gray-50 transition duration-150">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900" x-text="index + 1"></td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900" x-text="prodi.nama"></div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800" x-text="prodi.jurusan?.nama || 'N/A'"></span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800" x-text="prodi.kelas?.length || 0"></span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                    <button @click="openEditModal(prodi)" class="text-yellow-600 hover:text-yellow-900 transition duration-150">
                                        <i class="fas fa-edit mr-1"></i>Edit
                                    </button>
                                    <button @click="openDeleteModal(prodi)" class="text-red-600 hover:text-red-900 transition duration-150">
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            <h3 class="text-lg leading-6 font-medium text-gray-900">Tambah Program Studi Baru</h3>
                            <div class="mt-4 space-y-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Jurusan</label>
                                    <select x-model="newJurusanId" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                ...
                                <div>
                                    ...
                                    <input type="text" x-model="newNama" placeholder="Masukkan nama program studi" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div x-show="showEditModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            <h3 class="text-lg leading-6 font-medium text-gray-900">Edit Program Studi</h3>
                            <div class="mt-4 space-y-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Jurusan</label>
                                    <select x-model="editingJurusanId" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:border-transparent">
                                        <option value="">Pilih Jurusan</option>
                                        <template x-for="jurusan in jurusans" :key="jurusan.id">
                                            <option :value="jurusan.id" x-text="jurusan.nama"></option>
                                        </template>
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">
                                    Yakin ingin menghapus program studi "<span x-text="selectedProdi?.nama" class="font-medium text-gray-900"></span>"?
                                </p>
                                <p class="text-sm text-gray-500 mt-1">
                                    Program studi dari jurusan "<span x-text="selectedProdi?.jurusan?.nama" class="font-medium text-gray-900"></span>".
                                </p>
    ...
    function prodiData() {
        return {
            ...
            storeData() {
                ...
                .then(data => {
                    // Find the jurusan data to add to the new prodi
                    const jurusan = this.jurusans.find(j => j.id == this.newJurusanId);
                    data.jurusan = jurusan;
                    
                    this.prodis.push(data);
                    this.closeAddModal();
            ...
            openEditModal(prodi) {
                ...
                this.editingNama = prodi.nama;
                this.editingJurusanId = prodi.jurusan_id;
            ...
            updateData() {
                ...
                if (!this.editingJurusanId) {
                    alert('Pilih jurusan terlebih dahulu!');
                    return;
                }

                fetch(`/api/prodis/${this.editingId}`, {
                    method: 'PUT',
                    headers: { 
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || ''
                    ...
                    body: JSON.stringify({ 
                        ...
                        jurusan_id: this.editingJurusanId
                    }),
                })
                .then(res => {
                    if (!res.ok) throw new Error('Network response was not ok');
                ...
                .then(updated => {
                    const index = this.prodis.findIndex(p => p.id === this.editingId);
                    if (index !== -1) {
                        // Find the jurusan data to add to the updated prodi
                        const jurusan = this.jurusans.find(j => j.id == this.editingJurusanId);
                        updated.jurusan = jurusan;
                        
                        this.prodis[index] = updated;
                    }
                    this.closeEditModal();
            ...
            deleteData() {
                ...
                fetch(`/api/prodis/${this.selectedProdi.id}`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || ''
                    }
                ...
                .then(res => {
                    if (!res.ok) throw new Error('Network response was not ok');
                    this.prodis = this.prodis.filter(p => p.id !== this.selectedProdi.id);
